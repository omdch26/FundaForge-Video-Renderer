...
| Caption size + colour, second pass | **18 Aug 2026**, after watching the real S1E05 render Sri flagged two more things: captions were still close to `body`'s size, and the default (non-emphasis) caption colour was pure white — same as the card's own headline/body — reading as visually confused with on-screen copy. `TYPE.caption` dropped again, `{46,600}` -> `{34,600}`, now clearly under `body` (42). New palette token `fog` (`#B9C4CE`) added to `config/brand.json` — desaturated, distinct from `white` without being vivid — and `Captions.tsx`'s default word colour switched from `PALETTE.white` to `PALETTE.fog`. The muted/active opacity fade (0.38 -> 1 as each word is spoken) is unchanged — Sri explicitly wants that kept. `fog` isn't yet in `Brand_Guidelines.md` §4 upstream (brand.json states it mirrors that doc, not owns it) — worth a note there if this sticks. |
| Furniture brand mark restored | **18 Aug 2026**, same pass, took three goes to get right. (1) Sri compared the video against `S1E05_Carousel_Proof.pdf` and found the top-left brand mark missing (shrunk to a plain 3px bar) — restored as a flat lane-accent square, guessed off the carousel PDF's small compressed thumbnail. Wrong — Sri caught it ("cyan square blob... not my FundaForge logo"). (2) Checked the actual bare-glyph asset (`FundaForge_Icon.svg`) and inlined that instead, fixed brand colours regardless of lane. Still wrong — Sri's reference image was the *tile* variant (rounded obsidian square background, not transparent), and separately clarified the colours DO flip by lane, which the bare-glyph fixed-colour version didn't do. (3) Final: `FundaForgeMark` in `Furniture.tsx` now renders `FundaForge_Icon_Tile.svg`'s exact geometry (rounded obsidian tile, two-tone "F") with colours driven by `lane` — Season keeps the asset's own cyan-outer/amber-inner, Blueprint swaps to amber-outer/cyan-inner. Rendered both variants as PNGs via `cairosvg` and had Sri confirm visually before pushing (see chat) rather than guessing again. Lesson banked: for anything brand-visual, check the actual asset file AND render a preview for sign-off — three wrong guesses in a row on a two-colour icon is what "ask, don't assume" is for. |
| S1E05 first real audio + render pass | **18 Aug 2026, via VS Code (Windows).** First live ElevenLabs synthesis and first real Remotion render for this project. Two real bugs hit and fixed along the way, both worth knowing about for the next unit: (1) the installed `elevenlabs` SDK returns alignment data under different field names/units than `pipeline/audio.py` assumed (`characters`/`*_seconds`, start+end, vs. the expected `chars`/`*_ms`, start+duration) — fixed; the API call had already completed and spent credits before the crash, so the fix's retry hit cache, not a second charge. (2) Python 3.14 dropped the stdlib `audioop` module `pydub` depends on — fixed by installing the `audioop-lts` backport. Also found: amber-emphasis captions weren't carrying a `reason` field, so `pipeline/gates/brand.py` threw `AMBER_CAPTION_UNJUSTIFIED` on all 8 constraint captions — fixed in `pipeline/audio.py`, covered by a new test, re-validated clean. Separately, rendering itself failed once on Windows because `subprocess.run` can't launch `npm`/`npx` by bare name (they resolve to `.CMD` shims) without a shell — fixed by resolving the real executable via `shutil.which` instead of `shell=True`. Result: 9 scenes synthesized, ~88.2s of real speech (vs. the 68.3s authored guess — scenes retimed to the real recording, as designed), output at `out/drafts/S1E05/S1E05_v1.mp4` (1080×1920, 30fps H.264, AAC stereo, ~6MB). First-pass ElevenLabs cost: **592 credits** (~₹5.6 at the ₹1142-for-122,000-credits rate, if that's confirmed as the plan cost rather than a per-run charge — see `out/elevenlabs_credit_log.csv`). Reviewed directly (frames pulled from the actual file, not just described): title/diagram/trap/CTA cards all match brand — cyan Season accent, amber confined to the trap card's constraint text, "SUBSCRIBE + NOTIFICATIONS" CTA correct. A suspected caption/CTA-text overlap near the end was checked frame-by-frame (82–87s) and not reproduced — captions sit in the gap between the CTA card's text block and the bottom safe area, which is intentional (keeps clear of YouTube's own Shorts chrome), not an overlap. Nothing published, sent, or approved — sitting as `_v1` in drafts pending Sri's own watch-through, per the standing safety rules. Code committed to git from the VS Code session. |
