...
| Caption size reduced | **18 Aug 2026**, Sri flagged burned-in captions as too big for a text-heavy format. `TYPE.caption` in `layout.ts` was `{size: 60, weight: 700}` — 43% larger than `body`'s `{size: 42, weight: 400}` and heavier, so captions competed with the card's own copy instead of supporting it. Reduced to `{size: 46, weight: 600}` — just above body, clearly secondary to `headline` (76). Not yet visually verified against a real render at the time it was made; confirmed fine in the S1E05 first-pass review below (captions read as supporting text, don't dominate any sampled frame). |
